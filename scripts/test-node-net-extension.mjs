...
const folder = await mkdtemp(join(tmpdir(), "hibana-node-net-"));
...
try {
  ...
  const pkg = resolve("extensions/node-net");
  ...
  for (const path of [
    "dist/transport.wasm",
    "dist/NOTICE.txt",
    "dist/licenses/ring-0.17.14/LICENSE",
    "wit/world.wit",
    "compat/net.mjs",
    "compat/tls.mjs",
  ])
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      type: "module",
      dependencies: {
        "@hibana/node-net": `file:${join(folder, packed.filename)}`,
        hono: "4.13.7",
      },
    }),
  );
  ...
  await assert.rejects(
    access(join(application, "node_modules/@hibana/node-net/component")),
  );
  console.log(
