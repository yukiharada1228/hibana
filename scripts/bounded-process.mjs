...
export function runCommand(executable, args, {timeoutMs = 30000, signal, maxBytes = 8 * 1024 * 1024, input, cwd, env, diagnostics = false} = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let size = 0, chunks = [], outputTail = Buffer.alloc(0), failure, closed = false, killed = false, killTimer;
    const capture = chunk => {
      if (diagnostics) outputTail = Buffer.concat([outputTail, chunk]).subarray(-16384);
    };
    const kill = sig => {
      if (!child.pid) return;
    ...
    child.on('close', (code, sig) => {
      closed = true;
      if (code !== 0) failure ||= new Error(`Command failed (${sig || code})${diagnostics ? `: ${executable}\n${outputTail.toString()}` : ''}`);
      finish();
    });
  });
}
