import streams from "readable-stream/lib/ours/browser.js";
import { Buffer } from "buffer/";
import { connect as open, ipVersion } from "hibana:node-net/transport@0.1.0";

const { Duplex } = streams;
const CHUNK = 16 * 1024;
...
export class Socket extends Duplex {
  ...
  _step() {
    ...
    if (this._connected && (!this._tlsOptions || this._secure)) {
  ...
  _advanceConnection(state) {
    ...
    if (this._connected && this._tlsOptions && !this._tlsStarted) {
      this._handle.startTls(this._tlsOptions);
  ...
  _pullRead() {
    let progress = false;
    // push(false) propagates readable backpressure to WASI/rustls.
  ...
  _write(chunk, encoding, callback) {
    if (this._upgraded)
      return callback(
        error("ERR_SOCKET_CLOSED", "Socket ownership transferred to TLS"),
      );
    this._pendingWrite = {
  ...
  _destroy(cause, callback) {
    ...
    final?.(closed);
    // Report TLS errors before closing the adopted raw socket. Protocol clients
    // may treat that raw close as EOF and otherwise lose the certificate error.
    try {
      callback(cause);
    } finally {
  ...
  get readyState() {
    ...
    if (this.destroyed || this._upgraded) return "closed";
    if (this.readable && this.writable) return "open";
    return this.readable ? "readOnly" : this.writable ? "writeOnly" : "closed";
  }
}

export function adopt(raw, secure) {
  if (
    !(raw instanceof Socket) ||
    !raw._connected ||
    raw.destroyed ||
    raw._upgraded ||
    raw._tlsOptions ||
    ...
    throw error(
      ...
      "Upgrade requires an open, connected plain socket with drained buffers",
    );
  }
  deactivate(raw);
  secure._handle = raw._handle;
  raw._handle = null;
  raw._upgraded = true;
  secure._host = raw._host;
  secure._started = true;
  secure.pending = false;
  secure._rawSocket = raw;
  raw.once("close", () => {
    if (!secure.destroyed) secure.destroy();
  });
  activate(secure);
}
