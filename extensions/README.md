...
**単機能の Wasm 部品14個**、Node 互換6個、PostgreSQL の部品6個とプリセット2個を、独立した npm パッケージとして提供します。Hibana 本体への互換機能追加はなく、選んだ拡張とその依存だけをアプリへ合成します。
...
例えば SHA-256 だけを使う場合は、`@hibana/sha256` のみをインストールし、`hibana.json` に `"extensions": ["@hibana/sha256"]` と指定します。
...
[PostgreSQL の組み合わせ方](postgres-core/README.md)に従って `database/` に通信・認証を指定した JS、拡張マニフェスト、`package.json` を置きます。アプリの `hibana.json` は次の構成です。`./database` はアプリ内で用意するローカル拡張のパスです。
...
選択例は次のとおりです。接続先が要求する通信・認証方式に合わせます。
...
`extensions` と拡張の `dependencies` が Wasm 部品の合成範囲を決めます。プリセットで `ssl: false` を指定したり、アプリで Pool を使わなかったりするだけでは、プリセットの依存は外れません。構成を減らすときはローカル拡張の部品選択を変更します。

## 複数の機能をまとめて使うプリセット

既存アプリや複数方式をまとめて扱うアプリでは、次の入口も使えます。新規の最小構成は上記の個別選択から作ります。
...
使うプリセットを `"extensions": ["@hibana/postgres"]` のように指定します。両方のプリセットと上記のローカル拡張は同じ `pg` alias を提供するため、いずれか1つを選びます。

## 利用者向けの配布と移行

各パッケージは npm registry へ未公開です。配布者から必要なパッケージと依存パッケージの tarball（`.tgz`）を受け取ってください。

```sh
# vendor/ に必要なパッケージを各1バージョンずつ配置
...
下記のバージョンとその依存を導入して再ビルドします。古い tarball と依存を削除してください。通常の `pg` import と `extensions: ["@hibana/postgres"]` はプリセットとしてそのまま使えます。
...
現在のバージョンは `postgres`・`postgres-tcp` が0.7.4、`postgres-core`・`postgres-pool` が0.7.0、`postgres-transport-tcp`・`postgres-transport-tls` が0.7.3、`node-net`・`node-tls` が0.6.3、`node-stream` が0.7.0、`tls` が0.5.1です。他の部品のバージョンと、WIT の0.5.0インターフェースは維持しています。各 `package.json` の依存バージョンを揃えてください。配備済みの Wasm に修正を反映するには、アプリの依存更新・再ビルド・再配備が必要です。
...
tarball に JS・必要な Wasm／WIT・ライセンスを含めます。Node／PostgreSQL の tarball は Wasm を含みません。利用者のインストールに Rust や install hook は不要です。PostgreSQL のプリセットは公開部品の組み合わせだけを持ち、クエリ処理や認証コードを重複させません。

検証は `scripts/test-extension-boundaries.mjs`（単機能・非同梱）、`scripts/test-tcp-tls-extension.mjs`（実通信）、`scripts/test-postgres-compatibility.mjs`（DB・ORM）、`scripts/test-application-extensions.mjs`（汎用合成）。[設計](../docs/application-extensions.md)・[検証結果](../docs/postgres-compatibility.md)も参照してください。
