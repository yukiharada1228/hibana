...
function validateEnvironment(inputVars, inputSecrets) {
  ...
  if (Object.keys(vars).length + secrets.length > 64)
    throw new Error("At most 64 vars and secrets are allowed");
  if (Object.values(vars).some((value) => Buffer.byteLength(value) > 4096))
    throw new Error("Each var must be at most 4096 bytes");
  if (
    Object.entries(vars).reduce(
      (n, [key, value]) =>
        n + Buffer.byteLength(key) + Buffer.byteLength(value),
      0,
    ) > 32768
  )
