...
export async function init(directory = ".", { template = "hono", install = true } = {}) {
  ...
  const config = { name, vars: { GREETING: "Hello Hibana" }, limits: { memory_mb: 256, timeout_ms: 15000 } };
  const javascript = template === "hono" || template === "javascript";
  if (javascript) {
    config.main = "src/index.ts";
    const local = `file:${relative(root, sdk).split("\\").join("/") || "."}`;
    await writeFile(join(root, "package.json"), JSON.stringify({ name, private: true, type: "module", scripts: { dev: "hibana dev", build: "hibana build", deploy: "hibana deploy" }, ...(template === "hono" ? { dependencies: { hono: "^4.6.0" } } : {}), devDependencies: { "@hibana/cli": local }, engines: { node: ">=24" } }, null, 2) + "\n");
  } else {
