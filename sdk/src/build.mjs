  ...
  try {
    ...
    await writeFile(
      ...
      [
        ...
        // 全処理を 1 つの async respondWith に閉じる。x-hibana-env の除去に伴う request 再構築で
        // body ストリームが失われないよう、body 付き(POST 等)は先に arrayBuffer() で吸い出してから
        // 具象 body で作り直す（incoming request の body を new Request(req,{headers}) で持ち越すと
        // StarlingMonkey では読めなくなり、queue 配送の request.json() が無限待ちになる）。
        ...
        `        request = new Request(request.url, { method: m, headers: h, body: buf });`,
        `      }`,
        `    }`,
