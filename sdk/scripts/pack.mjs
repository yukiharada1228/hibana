...
const destination = new URL("../platform/manifests/migration/", import.meta.url);
const source = new URL("../../deploy/kubernetes/migration/", import.meta.url);
try { await access(source); }
catch (error) {
  if (error.code !== "ENOENT") throw error;
  // An already packed CLI can be packed again without the platform checkout.
  await access(new URL("job.yaml", destination));
  process.exit(0);
}
