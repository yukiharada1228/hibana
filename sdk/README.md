...
このソースは候補版`0.2.0-rc.2`です。npm に公開した版は `npx` で利用できます。未公開の候補を試す場合は[候補版の導入手順](../docs/release-candidate.md)を使ってください。
...
```bash
npx --yes @yukiharada1228/hibana@0.2.0-rc.2 init my-app
cd my-app
npm run dev
```
...
```bash
npm install -g @yukiharada1228/hibana@0.2.0-rc.2
hibana init my-app
cd my-app
npm run dev
```
...
```json
{
  ...
  "devDependencies": {
    "@yukiharada1228/hibana": "0.2.0-rc.2"
  }
}
```
...
```bash
npm install --save-dev --save-exact @yukiharada1228/hibana@0.2.0-rc.2
...
以降の `hibana ...` は、CLIを導入済みのHono・JavaScriptプロジェクト内では `npm exec -- hibana ...` として実行できます。グローバル導入済みなら直接 `hibana ...` を使えます。プロジェクト作成前やRust・Goでは `npx --yes @yukiharada1228/hibana@0.2.0-rc.2 ...` も使えます。
