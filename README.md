...
現在の公開候補版は[`0.2.0-rc.12`](https://github.com/yukiharada1228/hibana/releases/tag/v0.2.0-rc.12)です。npmからCLIを導入でき、GitHub Releasesからランタイム・コンソール・基盤を取得できます。OIDC認証、KeycloakのKubernetes構成生成、アプリの外部通信設定、`tail`とコンソールでのログ閲覧に対応しています。[版ごとの変更と基盤の導入手順](docs/release-candidate.md)を確認し、各コンポーネントのバージョンを揃えてください。
...
```bash
npx --yes @yukiharada1228/hibana@0.2.0-rc.12 init my-api
cd my-api
npm run dev
```
...
```bash
npx --yes @yukiharada1228/hibana@0.2.0-rc.12 init my-rust --template rust
npx --yes @yukiharada1228/hibana@0.2.0-rc.12 dev -c my-rust/hibana.json
```
