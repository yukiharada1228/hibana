...
Keycloakを同じKubernetesの専用namespaceで運用するための[設定生成・導入・既存ユーザー移行手順](../deploy/keycloak/kubernetes/README.md)を用意しています。`0.2.0-rc.6`以降の`platform init --with-keycloak`で生成し、Hibanaとは個別に配備します。KeycloakのDBは専用PVCへ保存します。
...
## 既存環境のissuerをHTTPからHTTPSへ変更する場合

Hibanaのユーザー照合には`issuer + sub`を使います。同じKeycloakのrealm・ユーザー・DBを保持しても、`OIDC_ISSUER_URL`だけを変更すると既存ユーザーは新しいissuerでログインできません。KeycloakのユーザーID、HibanaのユーザーID・所属・ロール・アプリを保持し、Hibana側の紐付けも移行します。メールアドレスからユーザーを作り直す必要はありません。

変更前に両DB・鍵・設定をバックアップし、旧issuerと移行対象ユーザーの対応を記録します。APIで移行する場合は、各テナントの管理者が期限を限定したAdmin APIトークンを事前に用意します。新しいissuer・callback・console URLへ切り替えた後、`PUT /users/{user_id}/oidc`に既存の`oidc_subject`を渡すと、現在のissuerへ紐付き、監査ログが残ります。APIトークンを使う移行担当者自身の紐付けは最後に更新します。

紐付けの更新は、そのユーザーの既存セッションとAPIトークンを失効させます。対象のCI用トークンも新たに発行し、ブラウザ・CLIは新URLで再ログインします。複数のControl Planeで新旧issuerを混在させず、切り替え中は管理操作を停止してください。既存Keycloakのクライアント設定は管理APIまたは管理画面で変更します。初回インポート用JSONの書き換えでは反映されません。

HTTPS接続と配備の確認は[別PCでの試用手順](https-pilot.md)を使います。
...
要求するOIDCスコープは`openid`と、Discoveryの`scopes_supported`に掲載されている場合の`email`です。`claims`パラメーターでもIDトークンの`email`を任意属性として要求します。メールを自動反映する場合は、認証基盤でIDトークンに`email`を含めてください。メール属性の提供はログインの必須条件ではありません。`email`スコープの公開がない場合は`openid`だけを要求します。ユーザー情報API・管理API・グループ属性・refresh tokenは使用しません。
...
9. 表示メールの自動反映には、クライアントの`email`スコープを割り当て、emailマッパーの「Add to ID token」を有効にします。標準のemailスコープはこの設定を含みます。
...
初期管理者と追加ユーザーは、作成時からOIDC identityを含む1行として登録します。identityが同一テナントで重複すると409を返し、ユーザーと監査記録の作成はまとめて取り消します。異なるidentityは同じ表示メールを持てます。[OIDC標準](https://openid.net/specs/openid-connect-core-1_0.html#StandardClaims)でもメールの一意性は保証されません。

## 表示メールの自動反映

OIDCログイン成功時に、検証済みIDトークンの`email`で、そのテナントに所属する本人の表示用メールを更新します。認証基盤で変更した後は、Hibanaからログアウトして再ログインしてください。コンソールの表示、`GET /auth/session`、`GET /users`に反映されます。既存セッションもセッション情報を再取得すれば更新後の値を読み取れますが、認証基盤の変更を常時監視する仕組みではありません。

メールは表示専用のため、`email_verified`がfalseまたは未提供でも反映します。メールによる本人照合・権限付与・アカウントの自動作成は行いません。ユーザーID・issuer・subject・ロール・認証世代は変更せず、既存セッションやAPIトークンも失効させません。メールを通知先や復旧先として信頼できることを保証するものではありません。

メールが未提供・空白のみ・320バイト超・制御文字を含む場合は、現在の表示を維持します。PKCE・所属・停止・認証世代を再確認してから、メール更新とトークン発行を同じトランザクションで確定します。変更時だけ`user_profile_updated`を監査記録へ追加し、メールの実値は監査詳細に含めません。

表示メールの自動反映に伴う`m20260922_000009_oidc_profile`は、旧メール一意インデックスを削除し、行・権限・既存トークンを保持します。異なるidentityで同じメールを保存できるためdownによる巻き戻しは拒否します。DBのバックアップを取得し、全Control Planeを停止して移行後、更新したControl Planeを起動してください。

## コンソール・CLI・CI

管理APIの権限は`read`（閲覧）、`deploy`（配備）、`admin`（管理）の3種類です。公開アプリのHTTPアクセスにはHibanaのAPI権限を使いません。旧`invoke`スコープは廃止し、指定された場合は拒否します。`m20260922_000010_management_scopes`は既存トークンから旧スコープだけを取り除き、他の権限・期限・失効状態を維持します。適用が必要な最新のマイグレーションと更新順序は[基盤DBの手順](database.md)を参照してください。
...
既存DBを使用する場合は、DBのバックアップを取得し、すべてのControl Planeを停止してから最新のマイグレーションまで適用します。`m20260920_000008_oidc_only`はパスワードハッシュ列、メールからの認証用SQL関数、旧トークン発行の互換トリガーを削除します。既存のテナント・ユーザー・トークン行・監査ログは保持し、全トークンと未交換のログイン結果を失効させます。削除したパスワードハッシュは復元できないため、downによる巻き戻しは拒否します。旧版への復旧には切り替え前のバックアップを使用してください。
