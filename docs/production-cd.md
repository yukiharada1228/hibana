...
バックアップ中の平文はCPのroot専用ディレクトリに一時保存し、通常の完了・エラーで削除します。強制終了で残った一時ファイルは次のバックアップ開始時に削除します。CPにはDB dump・圧縮・暗号化・復号検証用のディスク空き容量が必要です。各出力前に2GiBの余裕を確保できるか確認し、外部プロセスのファイルサイズ上限も設定します。容量不足や検証エラーになった場合は既存バックアップを保持し、配備を開始しません。
