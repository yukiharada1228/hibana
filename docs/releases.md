...
現在の公開候補版は[`0.2.0-rc.11`](https://github.com/yukiharada1228/hibana/releases/tag/v0.2.0-rc.11)です。以下のnpxの例をそのまま利用できます。npmの候補版は`next`で配布するため、バージョンを指定して取得してください。[基盤の更新条件と配布物](release-candidate.md)も確認してください。
...
```bash
npx --yes @yukiharada1228/hibana@0.2.0-rc.11 --version
npx --yes @yukiharada1228/hibana@0.2.0-rc.11 init hello
cd hello
npm run dev
# Ctrl+Cで停止
```
...
```bash
npx --yes --package=/path/to/hibana-cli-0.2.0-rc.11.tgz hibana runtime install --from /path/to/hibana-worker-0.2.0-rc.11-linux-x64 --sha256 HASH
npx --yes --package=/path/to/hibana-cli-0.2.0-rc.11.tgz hibana init hello --cli-package /path/to/hibana-cli-0.2.0-rc.11.tgz
cd hello
npm run dev
```
...
docker tag hibana-platform:0.2.0-rc.11-linux-amd64 registry.example.com/hibana/platform:0.2.0-rc.11-amd64
docker push registry.example.com/hibana/platform:0.2.0-rc.11-amd64
tar -xzf hibana-kubernetes-0.2.0-rc.11.tar.gz
```
