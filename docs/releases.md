...
現在のソース候補は`0.2.0-rc.4`です。以下のnpxの例は、その版のnpm公開後に使用できます。公開前は[候補版の導入手順](release-candidate.md)でtarballから取得・検証します。
...
```bash
npx --yes @yukiharada1228/hibana@0.2.0-rc.4 --version
npx --yes @yukiharada1228/hibana@0.2.0-rc.4 init hello
cd hello
npm run dev
# Ctrl+Cで停止
```
...
```bash
npx --yes --package=/path/to/hibana-cli-0.2.0-rc.4.tgz hibana runtime install --from /path/to/hibana-worker-0.2.0-rc.4-linux-x64 --sha256 HASH
npx --yes --package=/path/to/hibana-cli-0.2.0-rc.4.tgz hibana init hello --cli-package /path/to/hibana-cli-0.2.0-rc.4.tgz
cd hello
npm run dev
```
...
docker tag hibana-platform:0.2.0-rc.4-linux-amd64 registry.example.com/hibana/platform:0.2.0-rc.4-amd64
docker push registry.example.com/hibana/platform:0.2.0-rc.4-amd64
tar -xzf hibana-kubernetes-0.2.0-rc.4.tar.gz
```
