...
準備確認と実行用の読み込みは分離しています。`/prepare`・HEADによる確認は、ディスク上のコードが実行可能であることを検証し、HTTP用LRUの内容や使用順を変更しません。検証済みのファイル情報だけを最大256件保持し、同じファイルへの再確認ではコードを読み直しません。ファイルの削除・置換・変更時には再検証します。ディスクにない成果物をメモリだけのキャッシュヒットで公開可能とは判断しません。HTTP用LRUにコードを残すのは実際のリクエストだけです。
...
共有キャッシュの上限はバケット単位で2 GiB・256件です。Control Planeが内部の準備トークンとHMACを検証した上で保存し、保存前に古い更新日時のキャッシュから回収します。公開中の元Wasmや別のprefixは削除しません。共有キャッシュの古さは最終実行日時ではなく保存日時で判定します。書込みはPostgreSQLのadvisory lockでControl Plane間を直列化し、受信は各CPで1件・128 MiB+署名32バイトに制限します。共有キャッシュの欠落・改ざん・読取り失敗は元Wasmのコンパイルへフォールバックし、共有保存の失敗はローカルの準備成功を取り消しません。新しいデータの受信・検証・保存もWorkerのコンパイル枠内に収めます。

この上限は最新オブジェクトの論理サイズであり、S3の旧版・削除マーカー・Garageの物理的な回収待ちは含みません。障害時の遅延PUTなどで一時的に超過した場合は次回の保存時に回収します。キャッシュ用prefixで履歴を残す場合は、ストレージ側のライフサイクルも設定してください。共有キャッシュがあってもPod起動、ダウンロード、コード読込みには時間がかかり、起動時間がゼロになるわけではありません。Podごとの容量を超えたアプリも必要時に復元するため、全公開アプリの常駐は必要ありません。
