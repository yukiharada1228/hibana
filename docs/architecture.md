...
- Argon2 によるパスワードのハッシュ生成・照合は blocking タスクで行い、ログイン・ユーザー作成・テナント初期管理者作成で共通の枠を使い、CP プロセスあたり同時2件に制限する。ログインはアカウント照会前、作成は認証・入力検証後かつ DB トランザクション開始前に枠を確保する。満杯なら `429 login_capacity`（ログイン）または `429 password_capacity`（作成）と `Retry-After: 1` を返す。枠は blocking タスクが所有し、クライアント切断後も処理が終わるまで解放しない。受理した照合では、ユーザー不在・ロックアウト時もダミーハッシュを使う。
...
- 同名ヘッダーのバイト列と順序を Worker まで保持する。ASCII値は既存の `headers` と `additional_headers` に格納する。非ASCII値を含む名前は全値を順序どおりBase64url化して `encoded_headers` にも格納し、Workerでその名前の値を置き換える。ローカルdevも同じ変換を使い、旧形式のリクエストも読み取れる。
