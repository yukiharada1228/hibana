...
検証・コンパイル子プロセスは環境変数を消去して起動し、検証にはメモリ上限、コンパイルにはRayonのスレッド数だけを渡します。親は入力送信と出力受信を並行して進め、入力送信中から終了までの時間と受信中の出力サイズを制限します。キャンセル・異常終了でも子の回収が終わるまで同時実行枠を保持します。Linuxで検証子のメモリ上限を設定できない場合は、制限なしで続けず終了します。
