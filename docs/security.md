...
| 境界 | 実装・受入条件 |
|---|---|
| CLI → 管理 API | loopback 以外は HTTPS 必須。リダイレクトを拒否。保存したトークンは別のサーバーへ再利用しない。ログイン情報は `.hibana/` 内、ファイル権限 0600。Secrets は標準入力で渡す |
...
2. 不正 Component、無限ループ、メモリ / ハンドル大量生成、遅い HTTP、同時 cold start を負荷試験し、正常アプリへの影響を計測する。コンパイルは Worker 内の blocking task のため、専用プロセスへの隔離・総量制限は次の改善対象。
3. 本番イメージの digest 固定、OS を含む脆弱性検査、署名・出所確認、更新手順、鍵ローテーション、バックアップからの Secrets を含む復元試験を行う。依存監査だけでは全経路の安全性を証明できない。
4. コードと共有 vars の切替を原子的にする。現在の複数 API 操作では配備失敗後に vars だけ変更される場合があり、rollback はコードのみを戻す。
