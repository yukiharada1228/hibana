...
同時実行数はPostgreSQLの`pending`・`running`行だけで管理します。テナント単位のトランザクションロックで集計と受付行の作成を直列化し、複数のControl Planeから同時に受け付けても上限を守ります。完了・受付取消のコミットで枠が空くため、Redisカウンタの減算や定期的な再同期は不要です。旧設定`INFLIGHT_TTL_SECS`は使用しません。Redisは呼び出し頻度・ログインの制限と一回限りのトークンを管理します。

通信切断・タイムアウトを理由にゲストを自動再実行しません。`running`の実行は副作用が発生した可能性があり、受付枠も直ちには返しません。Workerの結果保存、または既存reaperによる回収を待ちます。既定のstuck判定は1800秒であり、実行中にPodが失われた場合の即時復旧は未実装です。DB障害で結果保存に失敗した場合も、再試行で完了するかreaperが回収するまで枠を保持します。
...
WorkerはDB・エンジン・HTTPの初期化後にreadyになり、アプリ全件の常駐は要求しません。キャッシュが空でも、最初のリクエストで共有キャッシュまたは元Wasmから復元します。共有先では公開中の版と直前のロールバック先を容量による回収から保護し、2 GiB・256件の枠は保護対象外の古いキャッシュだけに適用します。Pod内の上限は従来どおりで、Garageの必要容量は公開アプリ数と成果物の大きさに応じて増えます。コンパイル済みコードが共有先にあっても、転送・検証の時間は必要です。準備用Serviceは公開前の検証と明示的なメンテナンス確認に使用します。
