...
- CLIのログイン・アプリ作成・配備を3ステップで表示。初回は基盤と同じバージョンの `npx --yes @yukiharada1228/hibana@VERSION`、作成後の配備はプロジェクト内のCLIを使う`npm run deploy`を案内。グローバルインストールは不要。
...
外部通信はアプリ共通の許可件数を表示し、通信先の一覧は展開して確認できます。許可が空なら全バージョンの外部通信を拒否します。設定画面を利用できるアカウントには「通信先の設定を開く」を表示します。閲覧権限だけでも通信先の確認は可能です。削除できないバージョンの理由や登録日時の説明も、履歴内で必要なときに展開できます。
...
アプリの通信許可は常に全バージョン共通です。初期値は空配列（APIの`allow_outbound: []`）で、外部通信を全拒否します。管理者による変更は次の実行準備から適用され、配備・ロールバックでは許可を増減しません。旧バージョン単位の承認APIは廃止しました。

この設定は Wasm の通信権限です。Kubernetes の NetworkPolicy や基盤側の private-address 制限も満たす必要があります。CLI とコンソールはそれらを自動変更しません。

## Kubernetes への導入

候補版 `0.2.0-rc.8` の CLI・Control Plane と同じソースから作ったコンソールを使用してください。CLI 接続画面は npm の同じ版を案内するため、コンソールの公開前に `npm view @yukiharada1228/hibana@VERSION version` で取得できることを確認します。以前の公開版には新しい `/auth/session`・`/auth/logout` API がありません。
...
```sh
docker build -t registry.example.internal/hibana/console:0.2.0-rc.8 console
docker push registry.example.internal/hibana/console:0.2.0-rc.8
```
...
公開先設定は `APP_PUBLIC_ORIGIN` に統一しました。旧 `INGRESS_BASE_DOMAIN` が残っていると起動・導入前検証で拒否するため、削除してスキームを含む完全な origin を指定してください。CLI の `--ingress-domain` は廃止し、`--profile` は複数の接続先を保存する場合だけ使用します。
