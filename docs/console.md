...
自動採番されたバージョンは「自動 d26b94ca」のような短い識別子で表示し、登録日時はバージョン履歴に集約しています。`--version 1.0.0`などで指定した番号はそのまま表示します。完全な番号は履歴の「バージョンの詳細」と切り戻しの確認画面で確認できます。登録日時はその版を最初に配備した日時で、切り戻しても変わりません。保存するバージョン番号や CLI の指定形式は変わりません。
