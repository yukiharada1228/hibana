...
CLI・PC用Wasmtimeランタイム・Control Plane・Workerを`0.2.0-rc.5`に揃えた候補版です。npm 公開前はGitHub Actionsで作成した候補を取得し、検証環境へ導入します。公開後は `npx --yes @yukiharada1228/hibana@0.2.0-rc.5` を使用できます。正式なReleaseを公開するまで、候補版のGitHub Release URLによる自動取得は使えません。
...
`develop`上の手動実行、または`release/v0.2.0-rc.5`ブランチの同じコミットに対して、CI、Security dependencies、Hibana releaseの全ジョブが成功した候補を選びます。`RUN_ID`はそのGitHub release実行のIDです。Actionsの成果物は14日間保持されます。

```bash
gh run download RUN_ID --repo yukiharada1228/hibana \
  --name hibana-release-candidate --dir .local/release/0.2.0-rc.5
cd .local/release/0.2.0-rc.5
...
| 配布物 | ファイル |
| --- | --- |
| CLI | `hibana-cli-0.2.0-rc.5.tgz` |
| Kubernetesマニフェスト | `hibana-kubernetes-0.2.0-rc.5.tar.gz` |
| PC用ランタイム | `hibana-worker-0.2.0-rc.5-{darwin,linux}-{x64,arm64}`の4ファイル |
| 基盤イメージ | `hibana-platform-0.2.0-rc.5-linux-{amd64,arm64}.tar`の2ファイル |
| コンソールイメージ | `hibana-console-0.2.0-rc.5-linux-{amd64,arm64}.tar`の2ファイル |
...
npx --yes --package=./hibana-cli-0.2.0-rc.5.tgz hibana --version
npx --yes --package=./hibana-cli-0.2.0-rc.5.tgz hibana runtime install --from ./hibana-worker-0.2.0-rc.5-darwin-arm64 --sha256 HASH
npx --yes --package=./hibana-cli-0.2.0-rc.5.tgz hibana init hello --cli-package ./hibana-cli-0.2.0-rc.5.tgz
cd hello
npm run dev
...
docker tag hibana-platform:0.2.0-rc.5-linux-amd64 registry.example.internal/hibana/platform:0.2.0-rc.5-amd64
docker push registry.example.internal/hibana/platform:0.2.0-rc.5-amd64
