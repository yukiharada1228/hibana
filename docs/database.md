...
OIDC専用版は`m20260920_000008_oidc_only`まで適用します。ユーザーのissuer・subject・認証世代と、トークンの認証方式・発行時の認証世代を照合します。パスワードハッシュ列と旧認証のSQL関数・互換トリガーは削除します。切り替え時に既存トークンをすべて失効させ、再ログイン・再発行を要求します。行と監査履歴は保持しますが、削除したパスワードハッシュを復元できないためdownは拒否します。更新前にControl Planeを停止し、復旧には切り替え前のバックアップを使ってください。[切り替え手順](authentication.md)を参照してください。
