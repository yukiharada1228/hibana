...
以下は`0.2.0-rc.12`以降の仕様です。rc.9以前には履歴行の自動削除は含まれません。追加のDBマイグレーションは不要で、既存の完了時刻の索引を使用します。
...
### 共有コンパイル結果の保持（0.2.0-rc.13）

`m20260925_000014_compiled_retention`は、内部のハッシュ参照関数に直前のロールバック先を追加します。既存の版・公開先・元Wasm・認証情報は変更しません。新しいControl PlaneとWorkerの起動前に適用が必要です。更新中は旧Control Planeを停止してください。旧版の共有キャッシュ回収は公開版の保護に対応していません。Pod内のキャッシュ上限は変わりません。保護対象を外すdownは拒否します。
