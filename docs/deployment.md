...
`bash scripts/test-http.sh`は使い捨てPostgreSQL・RedisとローカルCP/Workerを使用します。空DBの初期化・旧DBへの適用拒否、権限不足、同時配備、公開失敗、HTTP応答のvars/Secrets、rollback、Secret削除・再作成、RLS・複合FKを確認します。S3はこのテストではHTTPスタブです。実Kubernetesの更新・実S3の切替・長時間負荷は別の受入試験です。
